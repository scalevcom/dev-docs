...
- `readme/custom-code/` records the custom code configured in the ReadMe dashboard.
...
- In **Appearance → Navigation**, leave page icons off, hide Discussions, and add Status after Changelog as a **Link URL** to `https://status.scalev.com`.
- In **Appearance → Changelog**, use the **Continuous** layout so each repo-managed post displays its opening summary.
- In **Appearance → CSS, JS, HTML → HTML → End of Body Tag**, use [`readme/custom-code/end-of-body.html`](readme/custom-code/end-of-body.html). This makes the Status URL open in a new tab because ReadMe renders custom desktop navigation URLs with `target="_self"`.
